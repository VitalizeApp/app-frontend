document.addEventListener('DOMContentLoaded', () => {
    ...
    const icons = { 
        hospital: '<i class="fas fa-hospital icone-svg" style="color: #D9534F;"></i>',
        clinica: '<i class="fas fa-clinic-medical icone-svg" style="color: #5CB85C;"></i>',
        posto: '<i class="fas fa-plus-square icone-svg" style="color: #F0AD4E;"></i>',
        upa: '<i class="fas fa-ambulance icone-svg" style="color: #5BC0DE;"></i>',
        mapPin: '<i class="fas fa-map-marker-alt icone-svg"></i>',
        phone: '<i class="fas fa-phone-alt icone-svg"></i>',
        globe: '<i class="fas fa-globe icone-svg"></i>',
        calendar: '<i class="fas fa-calendar-alt icone-svg"></i>',
        procedure: '<i class="fas fa-notes-medical icone-svg" style="color: var(--azul-claro-principal);"></i>'
    };
    ...
    function renderMapFacilityItem(facility, matchedProcedureName = null) {
        ...
        if (matchedProcedureName) {
            procedureHtml = `<p class="procedimento-correspondente">${icons.procedure} Oferece: ${matchedProcedureName}</p>`;
        }
        let typeIconHtml = icons[facility.type] || '<i class="fas fa-hospital-alt icone-svg"></i>';

        return `
            <div class="item-resultado" data-id="${facility.id}" ${matchedProcedureName ? `data-procedure="${matchedProcedureName}"` : ''}>
                <h3>
                    ${typeIconHtml}
                    ${facility.name}
                </h3>
    ...
    function displayMapSearchResults(results, searchType = 'facility') {
        ...
        document.querySelectorAll('#mapSearchResults .item-resultado').forEach(item => {
    ...
    function displayMapFacilityDetails(facility, matchedProcedureName = null) {
        ...
        if (matchedProcedureName) {
            detailsHtml += `<div class="detalhe-procedimento-correspondente">
                                <p>${icons.procedure} Este local foi encontrado porque oferece: <strong>${matchedProcedureName}</strong></p>
                           </div>`;
        }

        detailsHtml += `<div class="info-agendamento">
        ...
        mapFacilityDetailsEl.classList.remove('oculto');
    }
    
    function resetMapView() {
        if (mapSearchTermEl) mapSearchTermEl.value = '';
        if (mapSearchResultsEl) mapSearchResultsEl.innerHTML = '';
        if (mapFacilityDetailsEl) mapFacilityDetailsEl.classList.add('oculto');
        if (mapSearchTypeEl) mapSearchTypeEl.value = 'facility';
            ...
            if (navigator.permissions) {
                navigator.permissions.query({name:'geolocation'}).then(permissionStatus => {
                    if (permissionStatus.state === 'granted') {
                        locationRequestPromptEl.classList.add('oculto');
                        ...
                        locationRequestPromptEl.classList.remove('oculto');
                        userHasInteractedWithLocationPrompt = true;
                        updateMap(initialMapCenter.lat, initialMapCenter.lng, initialMapZoom);
                    } else {
                        locationRequestPromptEl.classList.remove('oculto');
                ...
                updateMap(initialMapCenter.lat, initialMapCenter.lng, initialMapZoom);
                 ...
                 if (mapFrameContainerEl) mapFrameContainerEl.innerHTML = '<p class="marcador-mapa">Serviço de mapa não disponível.</p>';
                 if(locationRequestPromptEl) locationRequestPromptEl.classList.add('oculto');
            }
